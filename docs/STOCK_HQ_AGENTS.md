...
`upsert_decision_review` and `list_decision_reviews` accept `book`. On the Stock HQ
connector (`mcp:tools`): omitted branch is Ivan's live book (`REAL`); `branch=CANDIDATE`
forces `book=PAPER`; explicit `book=PAPER` with `branch=LIVE` is the current-rules paper
book. `get_prompt(branch=CANDIDATE)` prepends the PAPER PASS brief (Pass A then Pass B).
An optional `mcp:shadow` connector still exists and is always PAPER. See
